...
@section('content')
    <section class="flex justify-center items-center h-full">
        <div class="border border-cyan-500 rounded-xl shadow-lg p-5 lg:p-10 w-full lg:w-3/4 mx-2">
            ...
            <form action="{{ route('addProductMethod') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="lg:flex lg:gap-5">
                    <div class="mb-5 basis-1/2">
                        <label class="block mb-1" for="name">Name :</label>
                        <input type="text" placeholder="Enter product name" class="bg-gray-200 w-full block py-3 px-5 rounded outline-cyan-500" name="name" id="name" required value="{{ old('name') }}">
                ...
                <div class="mb-5">
                    ...
                    <textarea class="bg-gray-200 w-full block py-3 px-5 rounded outline-cyan-500" placeholder="Enter product detail" name="detail" id="detail">{{ old('detail') }}</textarea>
                    @error('detail')<p class="text-red-400 mt-1">{{ $message }}</p>@enderror
                </div>
                <div class="lg:flex lg:gap-5">
                    <div class="mb-5 basis-1/2">
                        <label class="block mb-1" for="price">Price :</label>
                        <input type="number" placeholder="Enter product price" min="0" max="100000000" step="1" pattern="\d*" class="bg-gray-200 w-full block py-3 px-5 rounded outline-cyan-500" name="price" id="price" required value="{{ old('price') }}">
                        @error('price')<p class="text-red-400 mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div class="mb-5 basis-1/2">
                        <label class="block mb-1" for="photo">Photo :</label>
                        <input type="file" class="bg-gray-200 w-full block py-2.5 px-5 rounded outline-cyan-500" name="photo" id="photo" required accept=".png,.jpg,.jpeg">
                        @error('photo')<p class="text-red-400 mt-1">{{ $message }}</p>@enderror
                    </div>
                </div>
                <button type="submit" class="block w-full bg-cyan-500 rounded border border-cyan-500 text-white p-3 font-bold transition hover:bg-transparent hover:text-cyan-500">Add Product</button>
            </form>
